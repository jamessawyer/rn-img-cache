...
export class ImageCache {
    ...
    download(cache) {
        ...
        if (!cache.downloading) {
            const realPath = this.getPath(uri, cache.immutable)
            const path = realPath + "_tmp";
            // console.log(">>>>>rn-img-cache downloading [" + uri + "] to path [" + path + "]");
            ...
            cache.task.then(() => {
                ...
                // cache.path = path;
                RNFetchBlob.fs.mv(path, realPath).then((ret)=>{
                    if (ret) {
                        cache.path = realPath;
                        this.notify(uri);
                    }else{
                        console.warn("move file [" + path + "] to [" + realPath + "] failed!");
